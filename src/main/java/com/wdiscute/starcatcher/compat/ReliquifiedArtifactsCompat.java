...
public class ReliquifiedArtifactsCompat
{
    ...
    public static List<ItemStack> getBonusCatchItems(Player player, FishingBobEntity fbe)
    {
        ...
        for (int i = 0; i < bonusCount; i++)
        {
            ...
            ItemStack is = FishProperties.makeItemStack(fbe.rod, fbe.fpToFish, size, weight, percentile, false, player, false);
            items.add(is);

            FishCaughtCounter.awardFishCaughtCounter(fbe.fpToFish, player, 0, size, weight, percentile, false, true, false);
            if (player instanceof net.minecraft.server.level.ServerPlayer sp)
                sp.giveExperiencePoints(fbe.fpToFish.rarity().getXp());
        }
